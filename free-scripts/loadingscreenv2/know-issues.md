...
* **Wrong path in config.js** — Must exactly match the file location (e.g. `assets/background.mp4`).
...
3. If no errors appear, check that `pl_loadingscreenv2` is ensured at the very **top** of `server.cfg` — not at the bottom.
...
**Fix:**
1. Confirm `ensure pl_loadingscreenv2` is in your `server.cfg`.
2. Confirm the folder name on disk matches exactly — it is case-sensitive on Linux servers.
3. Check the server console for any errors from `pl_loadingscreenv2` on startup.
